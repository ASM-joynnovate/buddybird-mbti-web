...
export function Toast({ message, onDismiss, ...rest }: ToastProps) {
    ...
    return (
        <div className="pointer-events-none fixed inset-x-0 bottom-[calc(env(safe-area-inset-bottom)+1.5rem)] z-50 flex justify-center px-gutter">
            <AnimatePresence>
                {message !== null && (
                    <m.div
                        key="toast"
                        role="status"
                        aria-live="polite"
                        className="flex w-full items-center gap-3 rounded-card bg-(image:--gradient-cta) px-5 py-3.5 text-on-primary shadow-raise-primary"
                        {...motionProps}
                        {...rest}
                    >
