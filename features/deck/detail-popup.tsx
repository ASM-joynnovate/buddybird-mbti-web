...
import { PortraitWindow } from '@/shared/ui/portrait-window';
	...
	return (
		<div className="fixed inset-0 z-60 flex items-center justify-center p-gutter">
			...
			<m.div
				...
				className="relative z-1 flex max-h-[calc(100dvh-2.75rem)] w-full max-w-md flex-col rounded-card
									...
									onSelectType !== undefined ? (
										<button
											...
											className="inline-flex cursor-pointer items-center rounded-full
												bg-(image:--gradient-cta) px-3.5 py-1 font-display text-sm
												tracking-wider text-on-primary shadow-raise-bar-primary
										...
										<span
											key={matchCode}
											className="inline-flex items-center rounded-full bg-(image:--gradient-cta)
												px-3.5 py-1 font-display text-sm tracking-wider text-on-primary
												shadow-raise-bar-primary"
										>
											{matchCode}
										</span>
									),
