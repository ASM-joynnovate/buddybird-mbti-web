...
function axisSides(questionId: string, choiceId: string, weights: Partial<Record<Letter, number>>) {
	...
	for (const [letter, value] of entries) {
		...
		if (meta === undefined) {
			throw new Error(`Content invariant violated: ${questionId}/${choiceId} weights unknown letter "${letter}"`);
		}
...
for (const question of QUESTIONS) {
	...
	for (const axis of Object.keys(sidesA) as Axis[]) {
		if (sidesA[axis] === sidesB[axis]) {
			throw new Error(`Content invariant violated: ${question.id} choices are not opposite on axis ${axis}`);
		}
	}
}
