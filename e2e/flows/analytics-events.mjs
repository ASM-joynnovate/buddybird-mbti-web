...
export async function run() {
    ...
    const initialCount = evalJs(
        `(window.__analyticsEvents && window.__analyticsEvents.length) || 0`,
    )
    assert(
        Number(initialCount) === 0,
        `event buffer must be empty before test starts; got ${initialCount}`,
    )
    ...
    for (;;) {
        if (questionIndex >= MAX_QUESTIONS) {
            throw new Error(
                `analytics-events: answered ${MAX_QUESTIONS} questions without reaching result-root`,
            )
        }
    ...
    assert(
        testStartEvents.length === 1,
        `expected exactly 1 test_start; got ${testStartEvents.length}`,
    )
    ...
    for (let i = 0; i < qaEvents.length; i++) {
        ...
        assert(
            p.index === i,
            `question_answered[${i}].payload.index must be ${i}; got ${JSON.stringify(p.index)}`,
        )
    }
    ...
    assert(
        lastStartIdx < firstQaIdx,
        'test_start must appear before the first question_answered in the event stream',
    )
    ...
    assert(
        lastQaIdx < tcIdx,
        'test_completed must appear after the last question_answered in the event stream',
    )
